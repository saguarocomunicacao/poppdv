...
var marginVerticalContainer = 50;
...
export default class AuthScreen extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      config_empresa: {
        modelo_de_abertura: 'modelo_de_abertura1',
        modelo_de_login: 'modelo_de_login1',
      },
  ...
  _loginSuccesso(userData) {
    if(userData.id=="0") {
      ...
      if (metrics.metrics.MODELO_BUILD == 'pdv') {
        console.log('_loginSuccesso 1');
        Functions._carregaLojaCategorias(this);
        console.log('_loginSuccesso 2');
        Functions._carregaProdutosPdv(this);
        console.log('_loginSuccesso 3');
        Functions._carregaEventos(this);
        console.log('_loginSuccesso 4');
        Functions._carregaPdvConfig(this);
        console.log('_loginSuccesso 5');
      }
  ...
  render () {
    ...
    // console.log('config_empresa',this.state.config_empresa);

    // splash: 'NAO'
    // entrada: 'SIM'
    // modelo_build_personalizado: 'metrics'
    // modelo_de_abertura: 'metrics'
    // config_empresa.modelo_de_abertura: 'NAO'
    // config_empresa.modelo_de_login: ''

    if(metrics.metrics.MODELO_BUILD==='pdv_TESTE') {
      return (
      ...
      if (this.state.splash === 'SIM' && this.state.entrada === 'NAO') {
        ...
        if (this.state.config_empresa.modelo_de_abertura == 'NAO') {
        ...
        } else if (this.state.config_empresa.modelo_de_abertura === 'modelo_de_abertura4') {
          if (this.state.visibleFormQRCode===true) {
            return(
              <LoginQRCode
                ref={(ref) => this.formRef = ref}
        ...
        } else if(this.state.config_empresa.modelo_de_login==='modelo_de_login2') {
          return(
            <>
            <Animated.View style={[this.state.styles_aqui.FundoLogin1,{
              width: Dimensions.get('window').width,
              ...
              />
              {(() => {
                if (this.state.config_empresa.modelo_de_abertura === 'modelo_de_abertura1' || this.state.config_empresa.modelo_de_abertura === 'modelo_de_abertura2' || this.state.config_empresa.modelo_de_abertura === 'modelo_de_abertura6') {
                  return (
                    <>
                    {(!visibleForm && !isLoggedIn) && (
                      <>
          ...
          return (
            <View style={[this.state.styles_aqui.FundoLogin1,{width: Dimensions.get('window').width,height: Dimensions.get('window').height}]}>
              <ImageBackground source={{ uri: ''+this.state.ImgFundoLogin+'' }} style={styles.backgroundImage}>
              <View style={{backgroundColor: 'transparent', height: 100, marginTop: 150}}>
              <Image
                animation={'bounceIn'}
                duration={1200}
                delay={200}
                ref={(ref) => this.logoImgRef = ref}
                style={[styles.logoImg_modelo_login_1,{marginTop: 0, width: Dimensions.get('window').width - 70}]}
                source={{ uri: ''+this.state.LogotipoLogin+'' }}
              />
              </View>
              {(() => {
